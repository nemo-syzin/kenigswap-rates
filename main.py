...
async def _rapira_get_json(url: str, timeout: float = 15.0) -> Optional[Dict[str, Any]]:
    """
    Rapira: ТОЛЬКО прямое подключение, БЕЗ proxy
    """
    headers = {
    ...
    for att in range(1, MAX_RETRIES + 1):
        try:
            async with httpx.AsyncClient(
                ...
                proxies=None,     # ← принципиально отключено
            ) as cli:
                r = await cli.get(url)
            ...
            log.warning(
                "Rapira GET %s attempt %s/%s failed: %s",
                url, att, MAX_RETRIES, e
            )
            if att < MAX_RETRIES:
...
async def fetch_rapira_usdtrub_best() -> Tuple[Optional[float], Optional[float]]:
    ...
    try:
        ...
        if isinstance(items, list):
            ...
            if target:
                ask = _safe_float(target.get("ask") or target.get("sell"))
                bid = _safe_float(target.get("bid") or target.get("buy"))

                _last_rapira_usdtrub = (ask, bid)
                _last_rapira_ts = datetime.now(KAL_TZ)
                _last_rapira_status = "rates"
                return ask, bid

        _last_rapira_status = "empty"
        return None, None
    ...
    for att in range(1, MAX_RETRIES + 1):
        try:
            async with httpx.AsyncClient(
                ...
                if not row:
                    raise RuntimeError("table row not found")
...
async def send_rates_message(app):
    ...
    if gr_ask is not None and gr_bid is not None:
        lines.append(
            _inline_line([
                _seg("Покупка", gr_bid + KENIG_BID_OFFSET,  comma=True),
                _seg("Продажа", gr_ask + KENIG_ASK_OFFSET, comma=False),
            ])
        )
    else:
